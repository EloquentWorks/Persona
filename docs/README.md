[![EloquentWorks Persona](https://github.com/EloquentWorks/Persona/actions/workflows/tests.yml/badge.svg)](https://github.com/EloquentWorks/Persona/actions/workflows/tests.yml)

# EloquentWorks Persona

A lightweight public profile package for Laravel applications.

EloquentWorks Persona provides slug-based public profiles with display names, bios, avatars, banners, social links, custom links, visibility controls, published profiles, and view tracking.

## Documentation

- [Installation](installation.md)
- [Configuration](configuration.md)
- [Usage](usage.md)
- [Routes](routes.md)
- [Customization](customization.md)
- [Testing](testing.md)

## Contributing

Contributions are always welcome. Please refer to our [Contibution Guidelines](CONTRIBUTING.md) before contributing

## Security

If you discover a security vulnerability, please report it privately to the package maintainer instead of opening a public issue. Please view our [Security Policy](SECURITY.md) if you have found a security issue within Laravel Persona

## License

Laravel Persona is open-sourced software licensed under the [MIT license](LICENSE.md).
